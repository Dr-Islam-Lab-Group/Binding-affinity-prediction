...
def main():
    ...
    # Identify reference sequence (WT proxy)
    ...
    X_mut_tr = mut_X.iloc[tr_idx].values
    X_mut_te = mut_X.iloc[te_idx].values
    
    mut_selector = VarianceThreshold(0.0)
    mut_scaler = StandardScaler()
    
    Xm_sel_tr = mut_selector.fit_transform(X_mut_tr)
    Xm_sel_te = mut_selector.transform(X_mut_te)
