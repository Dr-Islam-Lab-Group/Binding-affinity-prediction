#!/usr/bin/env python3
"""
Mutation-aware XGBoost with two-stage architecture and early stopping.
...
Key features:
    ...
    - XGBoost with early stopping based on validation set
...
class Config:
    ...
    BEST_CFG_JSON = "outputs/xgb_gridsearch/best_config.json"
    OUTDIR = "outputs/xgb_grouped_mutation_delta"
    ...
    # XGBoost parameters (fallback if best_config.json not found)
...
def load_best_config() -> Tuple[Dict, int, int]:
    """
    Load best XGBoost configuration from gridsearch results.
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("XGBoost Mutation-Delta Training Pipeline")
    logger.info("=" * 70)
    ...
    base_model.fit(
        Xa_fit, ya_fit,
        eval_set=[(Xa_fit, ya_fit), (Xa_val, ya_val)],
        early_stopping_rounds=early_rounds,
        verbose=False
    )
    
    stage_a_best_iter = base_model.best_iteration
    ...
    mut_model.fit(
        Xm_fit, yr_fit,
        eval_set=[(Xm_fit, yr_fit), (Xm_val, yr_val)],
        early_stopping_rounds=early_rounds,
        verbose=False
    )
    
    stage_b_best_iter = mut_model.best_iteration
